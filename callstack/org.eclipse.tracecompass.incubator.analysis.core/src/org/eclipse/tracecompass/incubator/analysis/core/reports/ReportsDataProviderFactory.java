 ...
 */
public class ReportsDataProviderFactory implements IDataProviderFactory, ITmfDataProviderConfigurator {
    ...
    private static final String CONFIG_SOURCE_TYPE_ID = ID + ".configurator"; //$NON-NLS-1$
    private static final ITmfConfigurationSourceType CONFIG_SOURCE_TYPE;
    private static final String NAME = "Reports"; //$NON-NLS-1$
    private static final String DESCRIPTION = "Reports data provider"; //$NON-NLS-1$

    private static final String CONFIGURATOR_NAME = "Reports Configurator"; //$NON-NLS-1$
    private static final String CONFIGURATOR_DESCRIPTION = "Configure reports analysis"; //$NON-NLS-1$

    /** Constants for configuration parameters */
    private static final String REPORT_TYPE = "reportType"; //$NON-NLS-1$
    ...
    static {
        // Initialize configuration source type
        CONFIG_SOURCE_TYPE = new TmfConfigurationSourceType.Builder()
                .setId(CONFIG_SOURCE_TYPE_ID)
                .setDescription(CONFIGURATOR_DESCRIPTION)
                .setName(CONFIGURATOR_NAME)
                .build();

        // Register built-in providers
        ReportsDataProviderRegistry.registerProvider(new ContainerReportDataProvider());
    ...
    public Collection<IDataProviderDescriptor> getDescriptors(ITmfTrace trace) {
        ...
        fTmfConfigurationTable.column(trace.getUUID()).values().stream()
                .map(config -> {
                    ReportProviderType type = ReportProviderType.valueOf(nullToEmptyString(config.getParameters().get(REPORT_TYPE)).toUpperCase());
    ...
    public @NonNull IDataProviderDescriptor createDataProviderDescriptors(@NonNull ITmfTrace trace, @NonNull ITmfConfiguration configuration) throws TmfConfigurationException {
        ...
        fTmfConfigurationHierarchy.put(trace.getUUID(), config.getId(), (String) config.getParameters().get(PARENT_ID));
    ...
    public void removeDataProviderDescriptor(@NonNull ITmfTrace trace, @NonNull IDataProviderDescriptor descriptor) throws TmfConfigurationException {
        ...
        for (Map.Entry<String, ITmfConfiguration> entry : childrenToRemove) {
            @SuppressWarnings("null")
            IDataProviderDescriptor childDescriptor = new DataProviderDescriptor.Builder()
                    .setId(entry.getKey())
                    .setProviderType(ProviderType.NONE)
                    .setConfiguration(entry.getValue())
                    .build();

            // Recursively remove the child and its descendants
            removeDataProviderDescriptor(trace, childDescriptor);
        }

        // Now handle the current configuration
        removeConfiguration(trace, config);

        // Remove from provider registry if applicable
        IReportDataProvider provider = ReportsDataProviderRegistry.getProvider(getReportType(config));
        if (provider != null) {
            provider.removeDescriptor(trace, config);
        }

        // Finally remove from configuration table
     ...
     * <li>Parent must be a container report type</li>
    ...
    private ITmfConfiguration validateConfiguration(@NonNull ITmfTrace trace, @NonNull ITmfConfiguration configuration) throws TmfConfigurationException {
        ...
        // System.out.println("parentId: " + parentId + " Name: " +
        // configuration.getName());
        // System.out.println("fTmfConfigurationTable: " +
        // fTmfConfigurationTable.rowKeySet());
        // System.out.println("fTmfConfigurationHierarchy: " +
        // fTmfConfigurationHierarchy.columnKeySet());
        if (parentId == null || (!parentId.equals(ID) && !fTmfConfigurationTable.contains(parentId, trace.getUUID()))) {
            throw new TmfConfigurationException("Invalid parent ID");//$NON-NLS-1$
        }
        ...
        ReportProviderType parentType = parentId.equals(ID) ? ReportProviderType.CONTAINER : getReportType(fTmfConfigurationTable.get(parentId, trace.getUUID()));
        if (parentType != ReportProviderType.CONTAINER) {
            throw new TmfConfigurationException("Parent must be a container report type");//$NON-NLS-1$
        }
    ...
    private void loadConfigurations(@NonNull ITmfTrace trace) {
        ...
        supplPath = supplPath.addTrailingSeparator().append(CONFIG_SOURCE_TYPE_ID);
    ...
    public static IPath getConfigurationBasePath(@NonNull ITmfTrace trace, @NonNull ITmfConfiguration configuration) {
        ...
        while (currentConfig != null) {
            ...
            currentConfig = fTmfConfigurationTable.get(
                    fTmfConfigurationHierarchy.get(trace.getUUID(), currentConfig.getId()),
                    trace.getUUID());
        }
    ...
    /**
     * Get the report type from a configuration
     *
     * @param configuration
     *            the configuration
    ...
    private static ReportProviderType getReportType(ITmfConfiguration configuration) throws TmfConfigurationException {
        try {
            return ReportProviderType.valueOf(nullToEmptyString(configuration.getParameters().get(REPORT_TYPE)).toUpperCase());
        } catch (Exception e) {
            throw new TmfConfigurationException("Invalid report type"); //$NON-NLS-1$
        }
    }
}