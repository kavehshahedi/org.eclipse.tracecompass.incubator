...
public class ReportsDataProviderFactory implements IDataProviderFactory, ITmfDataProviderConfigurator, IReportDataProvider {
    ...
    private static final String DESCRIPTION = "Configure various types of reports (text, image, etc.)"; //$NON-NLS-1$

    private static final ITmfConfigurationSourceType CONFIGURATION_SOURCE_TYPE;
    private static final String CONFIGURATION_ID = "org.eclipse.tracecompass.incubator.analysis.core.reports.config"; //$NON-NLS-1$
    private static final String CONFIGURATION_NAME = "Reports Configurator"; //$NON-NLS-1$
    private static final String CONFIGURATION_DESCRIPTION = "Configure custom reports"; //$NON-NLS-1$

    /** Constants for configuration parameters */
    private static final String REPORT_TYPE = "reportType"; //$NON-NLS-1$
    private static final String REPORT_TYPE_DESC = "Should be {0}"; //$NON-NLS-1$
    private static final String PARENT_ID = "parentId"; //$NON-NLS-1$
    private static final String PARENT_ID_DESC = "Unique ID of the parent report. The parent must be an existing report. If not specified, the root report container will be assigned as the parent."; //$NON-NLS-1$
        ...
        for (IReportDataProvider provider : ReportsDataProviderRegistry.getProviders()) {
            ...
            allDescriptors.addAll(List.of(
                    new TmfConfigParamDescriptor.Builder()
                            .setKeyName(REPORT_TYPE)
                            .setDescription(NLS.bind(REPORT_TYPE_DESC, provider.getReportType().name().toUpperCase()))
                            .setIsRequired(true)
                            .build(),
                    new TmfConfigParamDescriptor.Builder()
    ...
    private void loadConfigurations(@NonNull ITmfTrace trace) {
        ...
        supplPath = supplPath.addTrailingSeparator().append(CONFIGURATION_ID);
